...
<p style="color:black; font-size:25px;">Subject Name: <%=subjectName +" [ "+subjectCode+" ]" %></p>

 </div>

</section>
  <section style="height:200px;">
  <div style="margin-top:-140px;margin-left:-110px;width:120%;border:1px solid red;height:350px;overflow-y:auto;">
   <table id="customers">
      <thead >
  ...
  <div class="loader2" style="margin-top:-17%;margin-left:45%;" id="myLoader"></div>
...
<div class="modalViewQuestionPaper"  id="myModalQuestionPaper">
    <div class="modal-contentViewQuestionPaper">
     ...
     <div style="border:1px solid skyblue;height:330px;overflow-y:auto;">
   <table id="customers">
      <thead >
       <th style="text-align:center;">Sl.No</th>
...
<div class="modalViewQuestionPaper1"  id="myModalQuestionPaper1">
    <div class="modal-contentViewQuestionPaper1">
      ...
      <!-- Modal Header -->
        ...
        <h4 class="modal-title" style="text-align: center;font-size: 20px; text-transform: none;">Update Attendance Details</h4>
     

      <!-- Modal body -->
      <div class="modal-body" style="text-align:center;">
   
     <span style="font-size:18px;">Change Status<strong class="text-danger" style="color:red;">*</strong></span> 
     <select  id="updateAttendance" class="box" style="font-size:15px;width:150px;text-align:center">
					<option>--Select--</option>
					<option value="1">Present</option>
					<option value="0">Absent</option>
				</select>
				<br><br>
        <button type="button" id="submitUpdateAttendance" class="view-btn" style="">Update</button>
        <button type="button" id="closeQuestionPaperModel1" class="view-btn" style="background-color:red;">Close</button>
...
<!-- View Question Paper Modal end -->      
  ...
  function myFunction(id){
	...
	$('#closeQuestionPaperModel1').click(function(event) {
		event.preventDefault();
  		myFunction(tempId);
        $('#updateAttendance')[0].selectedIndex=0;
		$('#myModalQuestionPaper1').hide();
		$('#myModalQuestionPaper').show();

	});
	
	 function fetchAllStudentAttendanceList(){
		  
		  $.ajax({
		      type: "GET",
		      url:"/College_Final_Year_Project/attendance?action=allStudentAttendanceList&departmentId="+fkDepartment+"&semesterId="+fkSemester+"&sectionId="+fkSectionId+"&subjectId="+fksubjectId+"&courseTypeId="+courseTypeId,
	 ...
	 function fetchStudentAttendanceList(id){
		 $('#myModal').show();
		  	//Calling Loader
		  	$(".loader1").show();
		  ...
		  $.ajax({
		      type: "GET",
		      url:"/College_Final_Year_Project/attendance?action=studentAttendanceList&departmentId="+fkDepartment+"&semesterId="+fkSemester+"&sectionId="+fkSectionId+"&subjectId="+fksubjectId+"&courseTypeId="+courseTypeId+"&studentId="+id,
		      success: function (data, textStatus, jqXHR) {
		    	  var JsonData= jQuery.parseJSON(data);
		    	  $('#Table1').empty();
			     ...
			     $("#Table1").html('<tr class="no-records"><td colspan="6" style="text-align:center;text-color:black">Sorry,No record found.</td></tr>');
		          ...
		          $(JsonData).each(function (index, item) {  
		        	  $('#Table1').append(
		        		     ...
		        		     '<td style="text-align: center;"><input type="submit" value="Update" onclick=myFunction1('+item.pkId+')  class="view-btn"  ></td>'+
		        		      '</tr>'
		        	  );
			      });
		    	 }
	 ...
	 function myFunction1(id){
		...
		$('#submitUpdateAttendance').click(function(event) {
			event.preventDefault();
			 $('#myModal').show();
			  	//Calling Loader
			  	$(".loader1").show();
	        $("#submitUpdateAttendance").prop("disabled", true);
			$('#myModalQuestionPaper1').hide();
			
			var value=$('#updateAttendance').val();
			
			    $.ajax({
				      type: "GET",
				      url: "/College_Final_Year_Project/attendance?action=updateAttendance&id="+tempId1+"&value="+value,
				      success: function (data, textStatus, jqXHR) {
				        ...
				        $('#updateAttendance')[0].selectedIndex=0;
				        $("#submitUpdateAttendance").prop("disabled", false);
						$('#myModalQuestionPaper1').show();
